...
| 항목 | 설명 |
|------|------|
| 에러 조건 | 물체가 로봇 팔 뒤쪽으로 이동하여 카메라에서 보이지 않는 위치로 이동됨 |
| 감지 | gripper width < 임계값 → ANOMALY |
...
- [O] error_injector.py: 시나리오 A (물체 오프셋 2~5cm) 에러 주입
- [O] error_injector.py: 시나리오 B (물체를 로봇 팔 뒤쪽으로 이동, 카메라에서 안 보이는 위치)
- [O] anomaly 감지 시 로봇 즉시 정지 (관절 속도 0) + 카메라 캡처 동작 확인
- [O] wrist 카메라: panda_hand 바깥(-X)에 마운트, finger 방향을 봄 — self-occlusion 해결
- [O] 시나리오별 카메라 이미지 검증:
  - A: wrist에 오프셋된 큐브 보임, overhead에서 로봇 팔 옆에 살짝 보임
  - B: wrist/overhead 모두 큐브 안 보임 (로봇 팔 뒤에 가려짐)
- [O] sim_api.py: FastAPI HTTP 서버 (:8000), 별도 스레드 + 큐 기반 시뮬레이션 루프 연동
- [O] sim_api.py: POST /execute_action → action 실행 → 결과 + 이미지 + 로그 응답
- [O] sim_api.py: anomaly 발생 시 Agent Server에 AnomalyReport 전송 (코드 작성, Agent Server 구현 후 검증)
- [O] main.py: --scenario normal|a|b 플래그, P&P 실행 → ANOMALY → execute_action 대기
- [O] 복구 P&P 전 gripper 완전 열기 (settle 판정)
- [O] Isaac Sim 단독 테스트: anomaly 감지 → execute_action 수신(curl) → 복구 P&P 실행 확인
...
- [O] tools.py: extract_coordinates (OpenCV 기반 좌표 추출) + execute_action (Isaac Sim HTTP POST)
- [O] vision/object_detector.py: HSV 빨간 큐브 검출 + 픽셀→월드 좌표 변환
...
- [ ] ReAct 루프 각 단계 구조화된 로그 저장 (추후 웹 대시보드 연동용)
