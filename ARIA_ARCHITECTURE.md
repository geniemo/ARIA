...
> System Architecture Document v2.4
...
| 비전 | OpenCV (좌표 추출) |
| 로봇 | Franka Panda (SingleManipulator + RMPFlowController) |
| 로봇 제어 | RMPFlow (내장 Franka config) + ArticulationAction (gripper) |
...
### 1.3 로봇 제어 방식: RMPFlow

PickPlaceController(Isaac Sim 내장) 대신 RMPFlow + IK Solver 기반으로 pick-and-place를 직접 구현한다.

- **매 스텝 제어 가능.** 목표 좌표 도달 여부를 직접 확인하고 다음 단계로 진행. phase 전환 타이밍을 완전히 제어할 수 있음.
- **phase별 체크포인트.** 각 phase 완료 후 센서 상태를 확인하고, 문제 발생 시 중단 → agent에 보고.
- **gripper 독립 제어.** `ArticulationAction`으로 gripper open/close를 직접 명령. 닫힌 후 width 확인이 자연스러움.
- **확장 용이.** 향후 mid-motion interrupt나 연속 모니터링 추가 시 아키텍처 변경 없이 확장 가능.

**Pick-and-Place phase 및 완료 조건:**

| Phase | 동작 | 완료 조건 | 이상 감지 |
|-------|------|----------|----------|
| approach | end-effector를 grasp 위치로 이동 | 목표 좌표 도달 (위치 오차 < 임계값) | - |
| close gripper | gripper 닫기 | gripper 동작 완료 (더 이상 움직이지 않음) | **gripper width < 임계값 → ANOMALY** |
| lift | 물체를 들어올림 | 목표 높이 도달 | - |
| move | place 위치로 이동 | 목표 좌표 도달 | - |
...
Isaac Sim이 Franka용 RMPFlow config를 내장하고 있으므로 별도 config 생성 불필요.
...
RMPFlow 기반 pick-and-place에서 grasp phase 완료 후 gripper width를 체크한다. 물체를 잡았으면 물체 크기만큼에서 멈추고, 못 잡았으면 끝까지 닫힌다.
...
├── control/
│   ├── rmpflow_controller.py    # RMPFlow 기반 모션 제어
...
PickPlaceController(Isaac Sim 내장)는 phase를 통으로 실행하여 mid-phase 개입이 제한적이다. RMPFlow는 매 스텝 목표 좌표를 제어할 수 있어:

- phase 전환 타이밍을 직접 관리 → phase별 체크포인트 삽입 용이
- gripper를 `ArticulationAction`으로 독립 제어 → grasp 성공/실패 판정이 자연스러움
- 향후 mid-motion interrupt, 연속 모니터링 확장이 아키텍처 변경 없이 가능
...
**1-2. 모션 제어**
- [ ] rmpflow_controller.py: Franka 내장 config로 RMPFlowController 초기화
- [ ] rmpflow_controller.py: 목표 좌표 입력 → 매 스텝 joint action 계산 동작 확인
- [ ] gripper_controller.py: ArticulationAction으로 gripper close 구현
- [ ] gripper_controller.py: ArticulationAction으로 gripper open 구현

**1-3. Pick-and-Place phase 관리 + 완료 조건**
- [ ] pick_place_task.py: approach phase — end-effector를 grasp 위치로 이동, 완료 조건: 목표 좌표 도달 (위치 오차 < 임계값)
- [ ] pick_place_task.py: close gripper phase — gripper 닫기, 완료 조건: gripper 동작 완료 (더 이상 움직이지 않음)
- [ ] pick_place_task.py: lift phase — 물체를 들어올림, 완료 조건: 목표 높이 도달
- [ ] pick_place_task.py: move phase — place 위치로 이동, 완료 조건: 목표 좌표 도달
- [ ] pick_place_task.py: open gripper phase — gripper 열기, 완료 조건: gripper 동작 완료

**1-4. 이상 감지 + 로그**
- [ ] anomaly_detector.py: close gripper 완료 직후 gripper width 체크 → 임계값 미만이면 ANOMALY
- [ ] pick_place_task.py: phase별 실행 로그 기록 (phase명, status, duration_steps, gripper_width 등)

**1-5. 검증**
- [ ] 정상 pick-and-place end-to-end 동작 검증 (approach → close → lift → move → open)
- [ ] 물체 없이 grasp 시도 → anomaly 감지 동작 확인
- [ ] **1주 시점 판단: RMPFlow 실패 시 PickPlaceController로 fallback**
